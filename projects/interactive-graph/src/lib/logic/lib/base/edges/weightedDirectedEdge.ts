...
export class WeightedDirectedEdge extends DirectedEdge implements Weightable {
    ...
    protected drawWeight(ctx: CanvasRenderingContext2D, grid: AbstractGrid) {
        ...
        ctx.fillText(this.getWeight() +""+ this.unit, (dy >= dx) ? 5*textSize/3 : 0
            , (dy >= dx) ? 0 : 5*textSize/3);
        ctx.restore();
